    ...
    for scan in range(9): # loop over scans
        for pix in range(25): # loop over pixels
            for ch in range(15): # loop over channals
                tbs[rec,scan,pix,ch]=data_in[rec,counter] # join the vale
                counter=counter+1

# get rid of missing elements
  # declare
tbs_new             =np.zeros((n_rec,9,25,13))
  # populate
tbs_new[:,:,:,0:5]  =tbs[:,:,:,0:5]
...
# normalize
  # declare
tbs_norm            =np.zeros((n_rec,9,25,13))
...
ss1.tofile('test_my_data_reordered.bin')

# now lets read it back and print and make sure there is a match
test2 = np.fromfile('test_my_data_reordered.bin', dtype='float32,(9,25,13)float32', count=-1)
x = test2[9][1]
print('-------------------------------')
print(x[0,0,:])
